...
export default class OneDrive extends React.Component<
  IOneDriveProps,
  IOneDriveState
> {
  constructor(props) {
  ...
  public render(): React.ReactElement<IOneDriveProps> {
    let fldrs = this.state.folders.map((fldr) => {
      return (
        <Folder folder={fldr} subFolderCallback={this.getSubFolders}></Folder>
      );
    });
    return (
      <div className={styles.groups}>
        ...
        <div className="recent-content">
          <ul className={styles.list}>{fldrs}</ul>
        </div>
        <div>
          <PrimaryButton
          ...
          <PrimaryButton
            className={styles.saveBtn}
            text="Mail Metadata"
            onClick={this.saveMailToContent}
            allowDisabledFocus={true}
          />
  ...
  private MailMetadataItems = {
    fields: {
      Title: "Test",
    },
  };

  private saveMailToContent = async () => {
    console.log("Metadata");
    const saveLocationDisplayNameSP = `SP ...> `;
    const response =await this.props.graphController.retrieveMailContent(
      this.state.parentFolder.driveID,
      this.state.parentFolder.id,
      this.props.mail,
      this.saveMailCallback
    );
     

    console.log("SPData : " + JSON.stringify(response));
  
    this.props.graphController.saveMailMetadataToSP(
      this.props.mail.id,
      saveLocationDisplayNameSP,
      this.state.parentFolder.webUrl,
      new Date(),
      this.MailMetadataItems
    );
  };
  ...
  private saveMailTo = () => {
    ...
    this.props.graphController
      ...
      .then((response: string) => {
        ...
        this.props.graphController.saveMailMetadata(
          this.props.mail.id,
