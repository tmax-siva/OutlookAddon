...
import { IFolder, IMessage } from "../model/IFolder";
...
export default class GraphController {
        ...
        response.value.forEach((item) => {
          ...
          if (item["@odata.type"] === "#microsoft.graph.group") {
            if (
              ...
              item.resourceProvisioningOptions.indexOf("Team") === -1
            ) {
              folders.push({
                id: item.id,
  ...
  public retrieveMimeMail = (
    driveID: string,
    folderID: string,
    mail: IMail,
    clientCallback: (msg: string) => void
  ): Promise<string> => {
    return this.client
      ...
      .get()
      .then((response): any => {
        if (response.length < 4 * 1024 * 1024) {
          // If Mail size bigger 4MB use resumable upload
          return this.saveNormalMail(
            driveID,
            folderID,
            response,
            Utilities.createMailFileName(mail.subject),
            clientCallback
          );
        } else {
          return this.saveBigMail(
            driveID,
            folderID,
            response,
            Utilities.createMailFileName(mail.subject),
  ...
  public retrieveMailContent = (
    driveID: string,
    folderID: string,
    mail: IMail,
    clientCallback: (msg: string) => void
  ): Promise<string> => {
    console.log("Test" + mail.id);
    return this.client
      .api(`me/messages/${mail.id}`)
      .get()
      .then((response): any => {
        let messageContent: Array<IMessage> = new Array<IMessage>();
        response.value.forEach((item) => {
          console.log("Test1:"+JSON.stringify(item.subject));
          messageContent.push({
            id: item.id,
  ...
  private saveNormalMail(
    ...
    const apiUrl =
      driveID !== folderID
        ? `drives/${driveID}/items/${folderID}:/${fileName}.eml:/content`
        : `drives/${driveID}/root:/${fileName}.eml:/content`;
    return this.client
      .api(apiUrl)
      .put(mimeStream)
      .then((response) => {
  ...
  public async saveBigMail(
    ...
    const apiUrl =
      driveID !== folderID
        ? `drives/${driveID}/items/${folderID}:/${fileName}.eml:/createUploadSession`
        : `drives/${driveID}/root:/${fileName}.eml:/createUploadSession`;
    return this.client
      .api(apiUrl)
      .post(JSON.stringify(sessionOptions))
      .then(async (response): Promise<any> => {
        try {
          const resp = await this.uploadMailSlices(
            mimeStream,
            response.uploadUrl
          );
          clientCallback("Success");
  ...
  private async uploadMailSlice(
    ...
    return await this.client.api(uploadUrl).headers(header).put(fileSlice);
  }

  public saveMailMetadata(
    mailId: string,
    displayName: string,
    url: string,
    savedDate: Date
  ...
  public saveMailMetadataToSP(
    ...
    console.log("metadata : " + listMetadata);
    if (this.saveMetadata) {
      const apiUrl = `https://graph.microsoft.com/v1.0/sites/n0zx.sharepoint.com,722813d1-3686-431c-935d-8c5a28aaa8bd,5763df42-a17f-4061-945e-afa54a51d7dd/lists/f2620a2d-32f5-45d8-892c-12a09760c80f/items`;
      const metadataBody = {
        "@odata.type": "microsoft.graph.openTypeExtension",
        extensionName: this.metadataExtensionName,
        saveDisplayName: displayName,
  ...
  public retrieveMailMetadata(mailId: string): Promise<any> {
    ...
    return this.client
      ...
      .then(
        (response) => {
          if (
            typeof response.extensions !== "undefined" &&
